...
"%NODE_EXE%" -e "const n=require('net');const s=n.connect(7890,'127.0.0.1');s.setTimeout(400);s.on('connect',()=>{console.log(1);s.destroy()});s.on('timeout',()=>{console.log(0);s.destroy()});s.on('error',()=>{console.log(0)})" > "%PCHK%" 2>nul
set "PROXYUP=0"
