...
/** 解析 Google 搜索结果页；结构变动时返回空，由其它引擎继续提供结果。 */
export function parseGoogle(html, limit = 5) {
  const out = [];
  const seen = new Set();
  const src = String(html ?? '');
  const re = /<a[^>]+href="(https?:\/\/[^"]+)"[^>]*>[\s\S]{0,800}?<h3[^>]*>([\s\S]*?)<\/h3>/g;
  let m;
  while ((m = re.exec(src))) {
    const url = m[1].replace(/&amp;/g, '&');
    const title = clean(m[2]);
    if (!title || seen.has(url) || /(^|\.)google\.[^/]+\/(?:search|url|preferences|accounts)/i.test(url)) continue;
    seen.add(url);
    const tail = src.slice(m.index + m[0].length, m.index + m[0].length + 1800);
    const snippet = clean(tail.match(/<div[^>]*>([\s\S]{30,900}?)<\/div>/)?.[1]);
    out.push({ title, url, snippet });
    if (out.length >= limit) break;
  }
  return out;
...
async function searchAllEngines(q, opts = {}) {
  ...
  const engines = ['bing', 'ddg', 'google', 'bilibili', 'moegirl']
...
export async function search(query, opts = {}) {
  ...
  // 引擎选择：默认 Bing；可传 engine:'ddg' 或 engine:'google'。
  // ⚠️ 两个引擎的退避/限流状态共用（都在搜索这件事上，没必要分开算）。
  const engine = ['ddg', 'google'].includes(opts.engine) ? opts.engine : 'bing';
  ...
  const url =
    engine === 'ddg'
      ...
      : engine === 'google'
        ? 'https://www.google.com/search?q=' + encodeURIComponent(q) + '&hl=zh-CN&num=' + (limit + 3)
  ...
  try {
    ...
    let results =
      engine === 'ddg' ? parseDDG(html, limit) : engine === 'google' ? parseGoogle(html, limit) : parseBing(html, limit);
