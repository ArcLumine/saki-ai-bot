 ...
 * 真人的"补充"有个很明确的特征：
 *   · **偶尔**发生（十次里两三次），不是每次
 *   · 只补主回复遗漏的**关键事实或条件**，不是临时多加一句反应
 *   · **一条就够**；连着补多条就是刷屏
 *
 * 所以这里的策略是**双重节制**：
 *   ① **概率闸**：不是每次都问模型（省调用，也保证"偶尔"）
 *   ② **模型闸**：问了它，它也可以说「没有要补的」（`无`）
 *   ③ **硬上限**：一次回复内最多追补 `maxPerReply`（默认 1）
...
export function shouldConsider(seed) {
  if (cfg().enable === false) return false;
  const p = Number(cfg().probability ?? 0.15);
 ...
 *
 * ⚠️⚠️ 2026-09-19 用户反馈：「追补的上下两句甚至完全意思相反」。
 *    根因：原来把「主回复 + 前面补过的话」拼成**一整段字符串**塞给模型
 *    （`spoken.join('\n')`），模型分不清哪句是"必须遵守的主张"、
 *    哪句是"我自己刚说的补充" —— 于是补充时容易**反着说**
 *    （比如主回复说"能"，补充说"不过其实不太行"）。
 *    改法：**把"主回复"单独标出来、明确要求补充不能反着说**，
 *    前面补过的话也单独列出来（避免第二条重复/反驳第一条补充）。
 *
 * @param {{mainReply:string, prior?:string[], context?:string, index?:number}} p
 * @returns {Promise<string>} 空串 = 不需要补充
 */
export async function askFollowUp(p = {}) {
  const mainReply = String(p.mainReply ?? '').trim().slice(0, 400);
  if (!mainReply) return '';
  // 问句结尾不追补（见 `endsWithQuestion` 的注释）
  if (endsWithQuestion(mainReply)) return '';

  const llm = await import('./llm.js');
  const idx = Number(p.index ?? 0);
  const prior = (Array.isArray(p.prior) ? p.prior : []).filter(Boolean);
  ...
  const sys = [
    ...
    '',
    '## 你现在的任务',
    '你刚在群里说完一段话（下面「主回复」）。现在做一次**遗漏审计**：',
    '只有主回复漏掉了会影响对方理解、判断或行动的关键内容，才允许补一句。',
    '',
    '## 铁律',
    '· ⚠️⚠️ **没有关键遗漏就只回一个字：「无」** —— **这是最常见的情况，别硬编**',
    '· ⚠️⚠️ **补充绝对不能和「主回复」的态度、结论或事实相反或自相矛盾**',
    '  （比如主回复说"能行"，补充不能说"不过其实不行/没那么行"；',
    '   主回复说"没事"，补充不能说"其实挺严重的"）——**先想清楚主回复的立场，再决定要不要补，方向必须一致**',
    '· ⚠️ **不要把主回复或前面已经补过的话换个说法再说一遍**（那不叫补充）',
    '· ⚠️ **不要为了凑而补**（「总之就是这样」「大概就这些」「换句话说」一律算「无」）',
    '· ⚠️ 真有补充的，只能是主回复遗漏的**具体事实、前提条件、限制、风险或必要行动**，且**和主回复站在同一边**',
    '· ⚠️ 单纯的吐槽、附和、态度、情绪加码、临时反应、玩笑，哪怕听起来顺口，也必须返回「无」',
    '· ⚠️ 如果只是同一个结论、同一个情绪或同一个建议的改写，必须返回「无」',
    '· ⚠️ 只补**一句**，20 字以内，不要分点、不要长篇',
    `· 这是第 **${idx + 1}** 次补充判断 —— 默认返回「无」；只有关键遗漏才可补充`,
    '· 直接给这句话（或者「无」），不要引号、不要解释',
  ].join('\n');

  const user = [
    p.context ? `# 群里最近的对话\n${String(p.context).slice(0, 800)}\n` : '',
    `# 你刚说完的「主回复」（补充必须和它立场一致，不能反着说）\n${mainReply}`,
    prior.length ? `\n# 你已经补过的话（别重复/别反驳这些）\n${prior.join('\n')}` : '',
  ]
    .filter(Boolean)
    .join('\n');
...
export function status() {
  return {
    ...
    maxPerReply: Math.min(1, Number(cfg().maxPerReply ?? MAX_PER_REPLY)),
