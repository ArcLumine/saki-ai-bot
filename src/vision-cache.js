...
async function fetchImage(call, file) {
  try {
    ...
    if (/^https?:\/\//i.test(String(p))) {
      const response = await fetch(String(p), { signal: AbortSignal.timeout(20000) });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      return Buffer.from(await response.arrayBuffer());
    }
    if (/^file:\/\//i.test(String(p))) {
      return readFileSync(new URL(String(p)));
    }
...
export function visionBlock(list) {
  ...
  if (hasImage) {
    guide.push(
      ...
      '',
      '**以下短评规则只适用于对方没有要求详细解读的普通发图闲聊。对方明确要求识图、读字、分析图片或排查截图时，必须按视觉识别结果完整回答，不能只说「景色不错」或故意省略对象。**',
      '',
      '**你要做的是「评论」，不是「描述」。** 这两件事完全不一样：',
      '',
      '| ❌ 描述（像 AI 识图） | ✅ 评论（像真人） |',
      '| --- | --- |',
      '| 「图里有一个主体和几个明显细节」 | 「这个细节做得挺讲究」 |',
      ...
      '6. **不要把所有户外画面都叫「景色」**：只有自然景观才这样称呼；看到建筑、室内、道路、车辆、人物或界面时，直接说实际对象。',
      '',
