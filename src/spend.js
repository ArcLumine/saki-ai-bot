 ...
 */
let epochDay = ''; // 生效的起点（自动推的 与 账本声明的，取更早的）
let autoEpochDay = ''; // 只按"文件生日"推出来的那个
let ledgerEpoch = ''; // 账本里自己声明的起始月（试用期）—— 见 applyLedgerEpoch()
let ledgerNote = ''; // 账本里的说明文字（原样存回文件）
try {
  let earliest = 0;
  const consider = (p) => {
    try {
  ...
  if (earliest) autoEpochDay = dayKey(new Date(earliest));
} catch (e) {
...
try {
  if (existsSync(FILE)) {
    ...
    state = { days: j.days ?? {} };
    ledgerEpoch = String(j.epoch ?? '').trim();
    ledgerNote = String(j.note ?? '').trim();
  }
} catch (e) {
...
/**
 * 把**账本自己声明的起始月**并进闸门：`state/spend.json` 顶层的 `epoch`（如 `"2026-08"`）。
 *
 * 为什么需要（2026-09-23 用户：「账本里边加个起始月份作为『上月』，标明这是试用期」）：
 *   闸门原来只按**文件生日**推起点 —— 而这个仓库是 9/21 才切分出来的，
 *   于是"9 月 10 号"那种较早的记录也会被当脏数据丢掉，月末工资单的"相比上月"就没得比。
 *   账本里显式声明一个起始月（**试用期**）就解决：它把闸门**放宽**到那个月。
 *
 * ⚠️ **只放宽、不收紧**（取更早的那个）—— 不能拿它去挡更新的数据。
 * ⚠️ 闸门本身的意义没变：仍然拦住"比账本起始月更早"的手工脏数据。
 */
function applyLedgerEpoch() {
  const e = /^\d{4}-\d{2}$/.test(ledgerEpoch)
    ? `${ledgerEpoch}-01`
    : /^\d{4}-\d{2}-\d{2}$/.test(ledgerEpoch)
      ? ledgerEpoch
      : '';
  epochDay = e && (!autoEpochDay || e < autoEpochDay) ? e : autoEpochDay;
}
...
function save() {
  try {
    ...
    const tmp = `${FILE}.tmp`;
    // ⚠️ 把账本自己声明的 `epoch` / `note` **原样存回去** —— 不保留的话，
    //    第一次落盘（比如剔脏数据那次 save()）就会把它们抹掉，起始月就白声明了。
    const payload = {
      ...(ledgerEpoch ? { epoch: ledgerEpoch } : {}),
      ...(ledgerNote ? { note: ledgerNote } : {}),
      ...state,
    };
...
export function salaryNote() {
  ...
  return [
    ...
    '· ⚠️ 别把「账户余额」和「工资」混着说 —— **余额不够是提醒 <主人> 去充值**（指着他），',
