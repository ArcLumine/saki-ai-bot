...
export function stripMarkers(text) {
  return (
    String(text ?? '')
      ...
      //      ⚠️⚠️ 而且**顺手把它两边的空白一起吃掉**：标记是**独立**写出来的
      //        ⇒ 那里本来就是**断句处** ⇒ 补一个「，」（"还拍啊 [拍照] 行" → "还拍啊，行"）。
      //      ⚠️⚠️ 收拾空白**只能在这个位置做**。早先我图省事写成对整段文本
      //        `[ \t]{2,} → ，` —— 结果把**正常排版里的两个空格也变成了逗号**
      //        （`test/face.js` 的「文本正常」那条当场抓到）。别再那么写。
      .replace(/[ \t]*\[\s*拍(?:照)?\s*(?:[:：][^\]]*)?\][ \t]*/g, (m) => (/^[ \t]/.test(m) ? '，' : ''))
      ...
      //    ⚠️ 2026-09-23 改：**这件事已经并到 ①-b 的标记替换里做了**（只在该剥的位置收），
      //       原来那两条对**整段文本**的 `[ \t]{2,} → ，` 会误伤正常排版 ——
      //       `test/face.js` 的「文本正常」抓到过，所以删掉了。
  );
}
