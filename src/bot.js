import { config, ROOT, KNOWLEDGE_DIR, paramsFor, zonedParts } from './config.js';
...
function sleepNightKey(d = new Date()) {
  ...
  const shifted = new Date(Date.UTC(p.year, p.month - 1, p.day) - 4 * 3600 * 1000);
 ...
 */
export function whereAmI(now = new Date()) {
  ...
  const dayKind = off
    ...
    : dow === 0 || dow === 6
      ? '**周末，不上学**'
      : '**上学日**（不放假）';
  const dateText = `${p.year}年${p.month}月${p.day}日 ${week}`;
  ...
  const lines = [
    `今天 ${dateText}，${dayKind}。`,
    `→ 现在 ${String(hh).padStart(2, '0')}:${String(p.minute).padStart(2, '0')}，按你的一天：${where}。`,
  ];
  if (schoolDay) {
    lines.push(
...
function timeText() {
  ...
  const week = ['周日', '周一', '周二', '周三', '周四', '周五', '周六'][p.weekday];
  ...
  const lines = [`## 现在的时间`, '', `${p.year}年${p.month}月${p.day}日 ${week} ${hh}:${mm}（${period}）`];
...
export class Bot {
  constructor() {
    ...
    // ⚠️ 2026-09-19 用户反馈：活跃群里"看起来像机器人一直在说话，答非所问"——
    //    根因是每个人各自触发的"主动接话"都直接 `enqueue`，同一个群会排成一串，
    //    生成一条要 10~20 秒，5、6 个人一起说话时排队排到几分钟后才轮到，
    //    答的时候话题早过去了。这里限制**同一个群同时只允许一条主动接话在排队/生成中**，
    //    多出来的直接放弃（不占位、不排队），避免越答越旧。见 onRaw 里的用法。
    this.voluntaryDepth = new Map(); // sessionKey -> 排队中的主动接话数量
  ...
  async onRaw(data) {
    ...
    if (payload.post_type === 'notice') {
      if (this.isPokeAtMe(payload)) {
        // ⚠️⚠️ 2026-09-20 用户要求：「私聊白名单外的人，不管发什么类型的消息都不回复」。
        //    戳一戳是 `post_type:'notice'`，**走不到下面 message 分支的私聊白名单检查**（会在这里 return）。
        //    以前这里直接调 `pokeBack`，而 `pokeBack` 只查群白名单 —— 私聊（没有 group_id）
        //    整段跳过 → **白名单外的人私聊戳一下也会被回复**（真实漏过）。
        //    现在：群里照旧（下面 pokeBack 自己查 allowGroups）；私聊必须先过白名单。
    ...
    // ⚠️⚠️ 2026-09-20 用户要求：「私聊白名单外的人，**不管发什么类型的消息都不回复**」。
    //    收口成 `privateAllowed()` —— 和 notice（戳一戳）、`handle()` 那条路**共用同一个判据**，
    //    以后任何"构造假私聊事件"的入口都能被这一道挡住。
    ...
    if (payload.message_type === 'group') {
      ...
      if (join) {
        ...
        // ⚠️ 排队上限：这个群已经有一条主动接话在排队/生成中的话，这条不再插队，
        //    直接放弃 —— 见构造函数里 `this.voluntaryDepth` 的注释。
        const vKey = history.sessionKey(payload);
        const vDepth = this.voluntaryDepth.get(vKey) ?? 0;
        const vMax = Math.max(1, Number(config.chat?.maxVoluntaryQueue) || 1);
        if (vDepth >= vMax) {
          log.info(
            `主动接话[${join.mode}]放弃：这个群已经有 ${vDepth} 条主动回复排队/生成中，再答就要答很旧的话了`,
          );
          return;
        }
        log.info(`主动接话[${join.mode}]，来自 ${payload.user_id}`);
        this.stats.received++;
        this.voluntaryDepth.set(vKey, vDepth + 1);
        this.enqueue(payload, { voluntary: join.mode })
          .catch((e) => log.error('主动接话出错:', e.message))
          .finally(() => {
            const d = (this.voluntaryDepth.get(vKey) ?? 1) - 1;
            if (d <= 0) this.voluntaryDepth.delete(vKey);
            else this.voluntaryDepth.set(vKey, d);
          });
        return;
      }
    }
  ...
  shouldJoinChat(event, { stickerIsNew = false, echoSticker = null } = {}) {
    ...
    //    真实案例：undefined 在群里回某人「@某群友 我看看有什么」，机器人却接了这条，
  ...
  sendFace(event, tag) {
    ...
    // ⚠️ 2026-09-21：把**实际要发的那张**和大小记下来（用户反馈「有时候发表情包会发送失败」）。
    //    表情是 base64 内联发的，超大动图会被 QQ **静默丢弃**（NapCat 回 ok、无报错），
    //    所以这条日志是唯一能看出"到底发的是哪张、多大"的地方。
    //    ⚠️ >400KB 时提成 warn —— 那种尺寸发出去大概率群里看不到。
    try {
      const bytes = statSync(p).size;
      const kb = Math.round(bytes / 1024);
      if (bytes > 400 * 1024) {
        log.warn(`发表情「${tag}」偏大（${kb}KB，base64 后约 ${Math.round((bytes * 4) / 3 / 1024)}KB）—— 可能发不出去`);
      } else {
        log.debug(`发表情「${tag}」（${kb}KB）`);
      }
    } catch {}
  ...
  scheduleHandle(event, meta = {}) {
    ...
    // ⚠️ 2026-09-19：私聊没有 @，正常聊天大多是「引用/指令」之外的普通消息 ——
    //    这类消息在私聊里改用 `privateWindowMs`（更长），让连续发的好几句先攒一起，
    //    再当一句来回；@她/引用她仍然走短窗口，不额外拖慢。
    let waitMs = atMe || hasReply
      ? Math.max(0, Number(b.windowMsAtMe ?? 300))
      : chatType === 'private'
        ? Math.max(0, Number(b.privateWindowMs ?? 4000))
  ...
  async handle(event, meta = {}) {
    ...
    //    放在 handle 最前面 —— 所有"构造假私聊事件"的入口（戳一戳、补看、未来的新功能）
    ...
    // ⚠️ 2026-09-19 用户反馈：主动接话排到自己时，群里可能早就刷过去了
    //    （生成一次要 10~20 秒，期间又来了一堆新消息）——这时候答出来的话
    //    看起来就是"没头没脑地接了句旧话"。见 `config.chat.voluntaryStaleAfter` 的注释。
    //    ⚠️ 只挡"主动接话"这一类：@ 她 / 私聊 / 关键词命中都是**明确冲她来的**，
    //       不管多晚都该回，不能因为"旧"就不回。
    if (voluntary && event.message_type === 'group') {
      try {
        const buried = recent.messagesAfterMe(event.group_id ?? '', event.message_id);
        const staleAfter = Number(config.chat?.voluntaryStaleAfter) || 8;
        if (buried >= staleAfter) {
          log.info(
            `[${history.sessionKey(event)}] 主动接话[${voluntary}]放弃：轮到它时已经被刷下去 ${buried} 条，答了也是接旧话`,
          );
          return;
        }
      } catch (e) {
        log.debug(`[主动接话过旧检查] 出错：${e.message}`);
      }
    }
    ...
    try {
      ...
      if (controller.signal.aborted) {
        ...
        // 余额耗尽只私聊主人，不把账户状态泄露给当前群或私聊对象。
        if (/402|Payment Required|Insufficient Balance/i.test(err.message)) {
          log.warn(`[${who}] 余额耗尽（402）→ 私聊主人提醒，不公开账户状态`);
          const c = balance.balanceComplaint({ force: true });
          const ownerId = String(config.ownerQQ ?? '').trim();
          if (ownerId) {
            const line = await balance.phraseLine(c.tier, c.line);
            await this.sendToPrivate(ownerId, line).catch((e) =>
              log.warn(`余额耗尽私聊提醒失败：${e.message}`),
            );
          } else {
            log.warn('余额耗尽但 ownerQQ 未配置，无法私聊提醒');
          }
          await this.sendText(event, '……我这会儿有点卡住了，等下再问一次', { reply: !sentFirst }).catch(() => {});
          return;
        }
  ...
  /**
   * 私聊总闸：**只有「私聊应答开着」且「这个人在白名单里」才返回 true**。
   *
   * ⚠️⚠️ 2026-09-20 用户要求：「私聊**白名单外**的人，不管发什么类型的消息都不回复」。
   *
   *    为什么单独抽成一个方法（而不是散着写）：
   *      · 普通私聊消息走 `onRaw` 的 message 分支；
   *      · **戳一戳**是 `post_type:'notice'`，**根本走不到**那条分支（在更上面就 return 了）；
   *      · 还有"补看 / 未来新功能"会**自己构造假的私聊事件**直接进 `handle`。
   *    三处各写一遍必然会漏（戳一戳就是漏的那个）。所以统一到这里，
   *    三处都调它 —— 加一处新入口时照着调就行，不会再漏。
   *
   * ⚠️ 语义：`privateChat` 是"私聊应答总开关"（关掉 = 谁都不回），
   *    `allowPrivateUsers` 是"允许名单"。两个都满足才放行（fail-closed）。
   *
  ...
  async pokeBack(payload) {
    ...
    //    （下面原来只查了「群白名单」，私聊整段跳过 —— 那是这次漏白名单的根因）。
  ...
  buildSystemPrompt(liveStatus = '', event = null, voluntary = null, currentText = '', vision = '', webSearch = '', forwardText = '', crashLog = '', solveMode = false) {
    ...
    // ⚠️ 2026-09-20：梗库（`knowledge/memes.md`）—— **按需**，没命中一个字都不注入。
  ...
  async sendChunk(event, raw, reply) {
    ...
    // ⚠️ 临时诊断（2026-09-21）：把**实际要发出去**的这条打出来，
    //    用来判断「接一句回两句」到底是「一段里两个意思」还是「真发了两个气泡」。
    //    每条气泡各打一行 → 数行数就知道分了几条。
    //    【诊断期临时加的，确认问题后删掉这行】
    if (text) log.info(`[发出气泡] ${text}`);
  ...
  startBalanceWatch() {
    ...
    const ownerId = String(config.ownerQQ ?? '').trim();
    if (!ownerId) {
      log.warn('工资余额监控：ownerQQ 未配置，只检查余额但不会发送提醒');
    } else {
      const tiersNow = balance.tierInfo();
      log.info(
        `工资余额监控已启动（每 ${Math.round(interval / 60000)} 分钟查一次，` +
          `档位：${tiersNow.map((t) => `${t.label}<${t.below}元`).join(' / ')}，仅私聊undefined ${ownerId}）`,
      );
    }
    ...
    const tick = async () => {
      try {
        ...
        if (!r.ok) {
          ...
          log.debug(`[工资] 余额 ${r.total} 元，但 ownerQQ 未配置，只记日志`);
        } else {
          const c = balance.balanceComplaint({ total: r.total });
          if (!c.need) {
            log.debug(`[工资] 余额 ${r.total} 元，这次不用私聊提醒`);
          } else {
            const line = await balance.phraseLine(c.tier, c.line);
            await this.sendToPrivate(ownerId, line);
            log.info(`[工资] 余额 ${r.total} 元 → 已私聊undefined提醒（${c.tier}）`);
          }
        }
      } catch (e) {
    ...
    // 启动后先等 90 秒，避免刚开机立即发通知。
    this.balanceTimer = setTimeout(tick, 90 * 1000);
    this.balanceTimer.unref?.();
  }

  /**
   * 往私聊发一条（定时提醒用，没有 event 上下文）。
   *
   * @param {string} userId
   * @param {string} text
   */
  async sendToPrivate(userId, text) {
    const target = String(userId ?? '').trim();
    if (!target) throw new Error('私聊收件人为空');
    const content = stripMdLite(String(text ?? '').trim());
