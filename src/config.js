...
/** 机器人进行日程、时段和提示词时间判断所使用的 IANA 时区。 */
export const getTimeZone = () => String(config.timeZone || 'Asia/Shanghai');

/** 取指定时区的日历字段；时间戳本身仍然使用真实 UTC 时间。 */
export function zonedParts(at = new Date(), timeZone = getTimeZone()) {
  ...
  return {
    ...
    hour: Number(value.hour), minute: Number(value.minute), weekday: weekdays[value.weekday] ?? 0,
  };
}

/**
 * **请求体里要不要带 `thinking: { type: 'disabled' }`**（2026-09-23 加）。
 *
 * 为什么要关：主模型（`deepseek-flash` 这类）是**推理模型** —— 不关思考它会先"想"几秒，
 * 而且思考**占 completion_tokens**（实测压缩故事线那次烧掉 7479/8000，正文被截断、
 * JSON 不完整、整次作废）。所以各处小调用都得关。
 *
 * ⚠️ 但这字段**不是所有服务都吃**：Google Gemini 的 OpenAI 兼容口会直接 **400**
 *    （`reasoning_effort: 'none'` 也一样 400 —— 2026-09-23 实测）。
 *    ⇒ 拿 Gemini 跑测验时，在 `config.yml` 写 `llm.thinking: off`，这一项就整个不发。
 *    **默认 `'disabled'` ＝ 跟以前一模一样**，DeepSeek 路线零变化。
 *
 * 用法：在请求体字面量里展开 —— `{ model, messages, ...thinkingField() }`
 */
export function thinkingField() {
  const mode = String(config.llm?.thinking ?? 'disabled').toLowerCase();
  if (['off', 'none', 'false', 'no'].includes(mode)) return {};
 ...
 *   会走"界面保存"那条路，**真的去写人设包里的 `persona.md` 和 `learned.md`**
 *   （2026-09-23 起 `persona.md` 住在 `personas/<id>/`，不在 `knowledge/` 了；
 *    `webui.js` 的 `knowPath()` 两边都认）。
 *
...
const DEFAULTS = {
  ...
  vision: {
    enable: true,
    /** 用哪个模型看图。留空时跟随 llm.model，并使用该模型的视觉能力 */
    model: '',
  ...
  chat: {
    ...
    /** 同一类模型错误再次在群里提醒前的冷却时间（毫秒） */
    errorHintCooldownMs: 5 * 60 * 1000,
    // ⚠️ 2026-09-19 用户反馈：活跃群里好几个人同时说话，机器人会一个一个「主动接话」，
    //    排队排到后面要等好几分钟才轮到，答的时候话题早过去了 —— 看起来像
    //    「一直在说话、还答非所问」。这里限制**同一个群同时只允许这么多条
    //    主动接话在排队/生成中**，超出的直接放弃（不排队），宁可不说也别答旧话。
    maxVoluntaryQueue: 1,
    // ⚠️ 同理：即使只排了 1 条，生成期间群里可能又冒出一堆新消息，
    //    等它答完时话题早变了。这里设一个「刷屏阈值」——真要发送前发现
    //    自己要接的那条后面已经被刷下去这么多条，就放弃不发了（配合 `messagesAfterMe`）。
    voluntaryStaleAfter: 8,
  },
  /** 生气、还击、否认自己是机器人 */
  attitude: {
    allowAnger: true,
...
function load() {
  ...
  // 旧配置只写 requireAtInGroup 时，不能被默认的 respondTo=2 遮住。
  const parsedTrigger = parsed.trigger;
  const hasOwn = (obj, key) => Object.prototype.hasOwnProperty.call(obj, key);
  if (
    parsedTrigger &&
    typeof parsedTrigger === 'object' &&
    !Array.isArray(parsedTrigger) &&
    !hasOwn(parsedTrigger, 'respondTo') &&
    hasOwn(parsedTrigger, 'requireAtInGroup')
  ) {
    cfg.trigger.respondTo = parsedTrigger.requireAtInGroup ? 3 : 1;
  }
  ...
  try {
    new Intl.DateTimeFormat('en-US', { timeZone: cfg.timeZone }).format();
  } catch {
    throw new Error(`timeZone 不是有效的 IANA 时区："${cfg.timeZone}"`);
  }
  ...
  cfg.attribution.timeoutMs = Math.max(2000, Number(cfg.attribution.timeoutMs) || 15000);
  ...
  cfg.context.batch.windowMs = Math.max(0, Number(cfg.context.batch.windowMs ?? 800));
      ...
      // ⚠️ 2026-09-19 用户要求：「私聊也要把连续发的好几句合成一句回答，不要一句一句回」。
      //    群聊的短窗口（900/1200ms）是刻意的（2026-09-13 用户要求"别为整合上文干等"），
      //    但私聊没有 @ 判断、且往往是一个人打字分几段发，正常间隔就会超过 1.2 秒 ——
      //    所以私聊单独给一个更长的窗口，专用于「没有引用/没有指令」的普通私聊消息。
      cfg.context.batch.privateWindowMs = Math.max(
        0,
        Number(cfg.context.batch.privateWindowMs ?? 4000),
      );
      ...
      cfg.context.batch.burstShortChars = Math.max(1, Number(cfg.context.batch.burstShortChars ?? 4));
      cfg.context.batch.burstQuietMs = Math.max(0, Number(cfg.context.batch.burstQuietMs ?? 5000));
      cfg.context.batch.burstGapMs = Math.max(0, Number(cfg.context.batch.burstGapMs ?? 3000));
  ...
  cfg.vision.model = String(cfg.vision.model ?? '').trim();
  ...
  // 同一类错误的群内提醒冷却；设为 0 表示每次都提醒，默认 5 分钟。
  cfg.chat.errorHintCooldownMs = Math.max(0, Number(cfg.chat.errorHintCooldownMs ?? 5 * 60 * 1000));
  // 同一个群里最多允许几条「主动接话」同时排队/生成（默认 1，避免活跃群里排队排到答旧话）
  ...
  // ⚠️ 2026-09-19 用户反馈"太频繁" → 默认从 0.15 降到 0.08
  cfg.selfFollowUp.probability = Math.min(1, Math.max(0, Number(cfg.selfFollowUp.probability ?? 0.08)));
  // 硬上限（用户指定「最多自己接自己五条消息」）
  cfg.selfFollowUp.maxPerReply = Math.min(2, Math.max(1, Number(cfg.selfFollowUp.maxPerReply) || 2));
  // 第 2 条起，还继续补的概率 —— 见 follow-up.js 的 `CONTINUE_P`
  // ⚠️ 2026-09-19 同上，默认从 0.5 降到 0.3
  cfg.selfFollowUp.continueProbability = Math.min(
    1,
    Math.max(0, Number(cfg.selfFollowUp.continueProbability ?? 0.3)),
  );
  // 两条补充话之间的间隔（像真人在打字）
...
/**
 * 收紧度没有全局设置：这是「某个群没有单独覆盖时」的起点。
 *
 * ⚠️ `config.chat.strictness` 是旧配置遗留，不能再作为这里的底稿。
 *    用户已经定过「默认 50，然后只保留分群的数据」；聊天运行时也用
 *    `DEFAULT_STRICTNESS = 50`。`paramsFor()` 必须和它保持同一个契约，
 *    否则管理界面删除覆盖后会显示旧全局值，而机器人实际仍按 50 走。
 */
export const DEFAULT_CHAT = { strictness: 50 };
...
export function paramsFor(kind, groupId) {
  ...
  if (kind === 'life') {
    ...
    // 收紧度只认「分群覆盖 → 默认 50」。`config.chat` 里的其它聊天设置仍可
    // 原样带回给管理界面，但旧 `config.chat.strictness` **故意不继承**。
    base = { ...DEFAULT_CHAT, ...(config.chat ?? {}), strictness: DEFAULT_CHAT.strictness };
  } else {
    base = config[kind] ?? {};
  }
