...
- **看不清就说看不清；看得清的一定要说。** ⚠️ 这两句是一对：
  「别猜」针对的是**你没法确认的东西**（这是谁、这是什么梗），
  **不是"你怕说错的普通东西"** —— 图上印着品牌、型号、店名、书名、字幕，
  看清了就照念，**不念才是失职**。
- **绝对不要编**：没看见的细节不补，模糊的地方就指明「这里看不清」。
- 如果有文字（站牌、路牌、UI、字幕、水印、聊天记录、对话框、**试卷、题目、书本**），
  **逐字念出来**。**截图里的文字常常就是答案本身**，比画面更重要。

## ⚠️ 看到**电子产品 / 包装盒 / 商品**时（2026-09-17 加，用户截图报的）

真实翻车：群友发了一张图 —— 左边是 **iPhone 16 Pro 的包装盒**（盒子上清清楚楚印着
「iPhone 16 Pro」），右边是个红色道具「苹果 78」，配文「111兄弟们苹果78直接拿下」。
识图回来的描述里**既没提手机、也没念盒子上的字**，她于是答
「**哪来的手机啊，不就一个抱西瓜的**」（把那个红外壳认成了抱西瓜的）。

**规矩**：这类图**必须先把"是什么东西"说清楚**，再描述画面：

- **包装盒 / 商品 / 数码产品**：念出**品牌、型号、印在上面的每一个字**
  （「白色盒子上印着 iPhone 16 Pro 和苹果 logo」）。
- **分不清是实物还是道具/玩具/图片**：**照实说"看起来像……但不确定"**，
  别把它当成别的东西（抱枕、玩偶、水果…）。
- **同一个画面里有好几样东西**：**逐个点名**（「左边一个包装盒，右边一个红色
  手机形状的物品」），别只描述其中一个。
- **图上的水印、字幕、玩梗文字**：照念 —— 那些常常说明这张图是**搞笑图/梗图**。
...
## ⚠️⚠️ 「不给名字」只有一份**黑名单**：**二次元角色**（2026-09-17 用户定）

**默认规矩是「看到什么说什么」** —— 品牌、型号、商品、地点、建筑、界面、
文字、人物长相，**全部照实描述**。
**只有下面这一类，才适用"不确定就别点名"：**

### 🚫 黑名单：判断「这是动漫 / 游戏 / 虚拟作品里的哪个角色」时

- **只在你有把握时才说这是谁。** 认角色靠**标志物**：特定的发饰、服装、
  随身物品、作品 logo、配色组合。
- **光凭「粉发少女」这种特征不许点名** —— 粉发角色成百上千，硬猜必然错。
- **不确定就别给名字**，只描述特征（「粉发、紫眼、拿着印考拉的手机」）。
  下游会拿你的描述去查资料，你不给错名字反而更有用。
- 如果图里的文字已经写明了是谁（截图里有名字），那就**照念**，那是确凿的。

### ✅ 黑名单之外 —— **一律照实说，看得清就必须念**

- **手机 / 电脑 / 相机 / 鞋 / 车 / 游戏机 / 包装盒上的品牌和型号**：
  看得清就一定要念出来（「白色包装盒上印着 iPhone 16 Pro」「背面有苹果 logo」）。
  这**不是"猜角色"**，是**把图上印的字念出来**，没有猜错的风险。
- **真实品牌 logo**（苹果 / 小米 / 华为 / 索尼 / 任天堂…）：认出来就说，
  **别因为"怕认错"而只写成"一个圆形 logo"或"某个品牌的标志"**。
- **实物 / 商品 / 道具**：直接说它像什么（「一个红色、骨头形状的道具手机」），
  **别认成完全无关的东西**（抱枕、玩偶、水果…）。
- **一句话记法**：**「别硬认角色」≠「别认东西」** ——
  前者认错了是编造，后者不认才是漏读。
...
群友发了一张图：左边是 **iPhone 16 Pro 的包装盒**（盒上清清楚楚印着型号），
右边是个红色道具「苹果 78」。识图**既没提手机、也没念盒子上的字**，
她于是答「**哪来的手机啊，不就一个抱西瓜的**」——
把那个红外壳认成了抱西瓜的，还反过来质疑对方。（用户截图报的。）
...
export async function describeImageDetailed(image, opts = {}) {
  ...
  // 留空时跟随主模型：config.vision.model 归一化后是空字符串，??/|| 都接不住，必须显式判断
  const configuredModel = String(opts.model ?? config.vision?.model ?? '').trim();
  const model = configuredModel || String(config.llm.model ?? '').trim() || DEFAULT_VISION_MODEL;
...
export function visionModelInfo() {
  return {
    visionModel: String(config.vision?.model ?? '').trim() || String(config.llm.model ?? '').trim() || DEFAULT_VISION_MODEL,
