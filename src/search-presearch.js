...
export async function preSearch(text, context = '', opts = {}) {
  ...
  // ── 用 Google 与维基补强多源结果 ──
  {
    const { search, wikiSearch, entityOf, isJunkResult } = await import('./search.js');
    const ent = entityOf(query);
    const better = [];
    // ① Google：用实体精确查询补强通用搜索结果。
    try {
      const google = await search(ent ? `"${ent}"` : query, { limit: 4, engine: 'google' });
      const ok = google.filter((r) => !isJunkResult(r));
      if (ok.length && (!ent || ok.some((r) => r.title.includes(ent)))) {
        better.push(...ok);
        log.info(`预搜索：Google 命中「${ent || query}」→ ${ok[0].title.slice(0, 30)}`);
      }
    } catch (e) {
      log.debug(`预搜索：Google 失败 ${e.message}`);
    }
