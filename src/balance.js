 ...
 *
 * `complainedByGroup` 保留用于兼容旧状态文件和预览；自动提醒现在只私聊主人，
 * 因此使用全局 `complained` 标记，避免向多个群重复泄露余额状态。
 */
...
export function balanceNote() {
  ...
  return [
    ...
    '| | 问题在谁 | 你该说什么 |',
    '| --- | --- | --- |',
    '| **余额低**（现在这条） | **在undefined身上** —— 账户是他管的，该他去充 | 「账上见底了，你充一下」 |',
    '| 工资（另一套话术） | 在"你挣多少、够不够花" | 「工资该发了」「够不够花」 |',
    '',
    '**现在说的是第一条：提醒他充值。**',
    '',
    '· ✅ 私聊提醒主人「undefined，账上快见底了，记得充点」「undefined，该充钱了」',
    '  ⚠️ 这条只会私聊给主人，不能发到群里。',
    ...
    '· ❌ 别诉苦式地念一遍账单，一两句就够',
    '',
    '一句话：**私聊undefined提醒他充值，语气是打趣，但不公开余额状态。**',
  ].join('\n');
}
 ...
 *
 * ⚠️ 三道验收（**短 / 带undefined / 不报数字**）一条都不能省：
 *    · 太长 → 像系统通知，不像群里随口一句话；
 *    · 没有「undefined」→ 主人私聊里没有明确的称呼；
