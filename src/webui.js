 ...
 * 知识文件名的安全校验。
 *
 * ⚠️ 允许四类路径：知识根目录、`groups/<纯数字群号>.md`、`anime/<安全库名>.md`、
 *    `sensitive/<安全文件名>.md`（敏感词库 —— 独立于共用库和动画库）。
 *    四类都只允许**恰好一层固定子目录**，文件名只收 `[\w.-]`；`..` 一律拒绝。
 *    动画库是后来加的，之前漏掉 `anime/`，列表点得开却读取 400，预览自然只剩空白。
 *
...
function facesWithMeta() {
  ...
  for (const f of idx.faces ?? []) counts.set(String(f.tag), (counts.get(String(f.tag)) ?? 0) + 1);
  const seen = new Map();
  return (idx.faces ?? []).map((f) => {
    ...
    const variant = (seen.get(tag) ?? 0) + 1;
    seen.set(tag, variant);
    return { ...f, bytes, exists: existsSync(p), variant, variantCount: counts.get(tag) ?? 1 };
  });
}
...
const routes = {
  ...
  'GET /api/knowledge/list': async (_req, res) => {
    ...
    try {
      ...
      //    所以它的说明在下面列目录时现算。
      const mk = (dir, n, from, display = n) => ({
        name: display,
        desc:
          // ⚠️ 敏感词走 `sensitive/<名>.md`：DESC 表按完整相对名登记，
          //    basename 查不到 —— 先按展示名查，再回退 basename。
          (DESC[display] ?? DESC[n] ?? '') +
      ...
      try {
        ...
        if (existsSync(sdir)) {
          for (const n of readdirSync(sdir)) {
            if (!n.endsWith('.md')) continue;
            files.push(mk(sdir, n, 'knowledge', `sensitive/${n}`));
          }
        }
      } catch {}
  ...
  'POST /api/knowledge/draft-anime': async (req, res) => {
    try {
      ...
      log.warn(`起草动画库失败：${e.message}`);
      send(res, e.bad ? 400 : 500, { ok: false, error: e.message });
    }
  },

  /**
   * 新建动画库文件：与通用「保存知识文件」分开。
   * 这样只能创建新的 `anime/<安全库名>.md`，绝不会用新建入口覆盖已有库。
   */
  'POST /api/knowledge/create': async (req, res) => {
    try {
      ...
      if (!/^[a-z0-9][a-z0-9._-]{0,31}$/.test(name) || name.includes('..')) {
        return send(res, 400, { ok: false, error: '库名只能含小写字母、数字、点、横线或下划线，且必须字母数字开头' });
      }
      const content = String(b.content ?? '').trim();
      if (!content) return send(res, 400, { ok: false, error: '动画库正文是空的' });
      if (content.length > personaDraft.MAX_ANIME_CHARS) {
        return send(res, 400, { ok: false, error: `动画库不能超过 ${personaDraft.MAX_ANIME_CHARS} 字` });
      }
      const dir = join(KNOW, 'anime');
      const target = join(dir, `${name}.md`);
      if (existsSync(target)) return send(res, 409, { ok: false, error: `动画库「${name}」已经存在，请直接打开修改` });
      mkdirSync(dir, { recursive: true });
      writeFileSync(target, content, { encoding: 'utf8', flag: 'wx' });
      const info = reloadKnowledge();
      log.info(`知识库新建动画库 ${name}.md（${content.length} 字），已热重载`);
      send(res, 200, { ok: true, name: `${name}.md`, reloaded: info });
    } catch (e) {
      if (e.code === 'EEXIST') {
        return send(res, 409, { ok: false, error: `动画库「${name}」已经存在，请直接打开修改` });
      }
      log.warn(`新建动画库失败：${e.message}`);
      send(res, 500, { ok: false, error: e.message });
    }
  },
