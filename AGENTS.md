...
## 🕘 一律按 **UTC+8** 跑（2026-09-22 用户定）

**用户原话**：「以后跑所有的东西都记得按照 UTC+8 的时区跑，对不上你就自己换算一下。」

- **跑任何 `node`（回归 / 验收 / 工具 / 探针）之前，先设时区**：

  ```powershell
  $env:TZ = 'Asia/Shanghai'
  ```

  ⚠️ 实测有效：`test/holiday.js` 在这台机器（`Central Standard Time`）上 **12 项失败**，
  设成上海后降到 **7 项** —— 那 5 项纯粹是**时区造成的假失败**，不是代码问题。

- **PowerShell 自己的 `Get-Date` 不吃 `TZ`** —— 它永远走机器时区（本机 `Central Standard Time`，
  夏令时期间 UTC-5）。需要"现在几点"时**自己换算**
  （⚠️ PS 7 的方法名是 `ConvertTimeBySystemTimeZoneId`，**不是** `…BySystemZoneId`，后者只有 5.1 有）：

  ```powershell
  [TimeZoneInfo]::ConvertTimeBySystemTimeZoneId((Get-Date), 'China Standard Time')
  ```

  当前这台机器：机器本地 ↔ UTC+8 相差 **+13 小时**（实测机器 09-22 18:41 → UTC+8 **09-23** 07:41，
  注意**日期会跨天**）。

- ⚠️ **别跟机器人自己的时区混**：机器人**不吃环境变量 `TZ`**，它读 `config.yml` 的 `timeZone`
  （现在是 `Asia/Tokyo`，+9）—— 于是一个"今天的账"可能落进 UTC+8 的明天。`TZ` 只影响
  **测试 / 脚本里的原生 `Date`**；写"今天 / 明天"这类判断前，先想清楚说的是**哪个时区的今天**。
...
- 这两个验收套件自己还有两个坑（详见 `TODO.md`）：
  **① 不改白名单** ⇒ 要临时把假群 `200000001` / 假号 `10000001` 加进 `config.yml`，否则她「当没看见」；
  **② `ask-attitude.js` 只认带引号的 `accessToken`** ⇒ token 也得临时加引号。
- 观察记录（2026-09-23，供以后选型参考）：Gemini lite 答**客服问题**干脆、还挺像她
  （「白名单报游戏 ID，找undefined批」）；但**服主/群友的口吻分层**比 DeepSeek 弱（两侧措辞几乎一样），
  偶尔会留个没闭上的「（」。
...
- ⚠️ 代码两侧都认（`knowledge.js` / `webui.js` 的 `knowPath()` / `personaDataFile()` 都是
  ...
  **硬证据是 `node test/prompt-snapshot.js` 逐字节一致**。
- `knowledge/templates/` **已整个删除**（2026-09-23 用户定）：它是老布局的重复副本，
  **没有任何代码读它**（删前用 `git grep` 核过）。将来要给新装用户模板，正路是 `personas/_template/`。
...
> 📖 **要更新 / 发版时先读对文档**（维护者侧）：同步与合并（从上游拉下来并进来）→ `GIT-WORKFLOW.md` §①；
> 公开快照的构建与推送 → 同文件 §④。**服务器 / 其他机器 → 只读 `UPDATE.md`**（它随公开快照发布；
> `GIT-WORKFLOW.md` 是维护者本地文档，**公开副本里没有**，找不到它属正常）。
...
- 两个远端（**2026-09-23 改正，别再指错**）：
  · **本体（只许拉）**：`https://github.com/hzyzhzy/saki-ai-bot` —— `SakiBot_UpStream` 镜像的
    `upstream`（2026-09-23 由 `origin` 改名）**直连它**；**Testing 不直连 GitHub** —— 它的
    `upstream` 指向**本地镜像文件夹**（内容=本体，离线可用）。两个 remote 的 **push 都已锁死**
    （`remote.upstream.pushurl = DISABLED`，手滑 `git push upstream` 直接报错）；
    **镜像里没有 `origin` 这个名字**。同步流程见 `GIT-WORKFLOW.md` §①。
  · **我的 fork（只许推）**：`https://github.com/ArcLumine/saki-ai-bot` —— Testing 的 `origin`，
    用来收 `testing`；**永远不要 fetch / merge 它（永不 `merge origin/*`）** —— 2026-09-23 就是
    被它坑的：它停在建 fork 时的旧快照 `63b3726`，本体 09-22 / 09-23 的新提交没进来
    （详见 `TODO.md` 的 `### 13`）。推之前**必须**先过 `TODO.md` 的历史体检 + 内容过目。
    ⚠️ **别点 VS Code 的「发布分支 / Publish Branch」**：它会让你选 remote，选到 `origin`
    就把**未体检**的 `testing` 推上公开 fork。
