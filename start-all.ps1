...
# ⚠️ NapCat 目录不要写死绝对路径：默认 = 本项目上一级下的 napcat\NapCat.Shell，
#    也可以用环境变量 NAPCAT_DIR 覆盖。
if ($env:NAPCAT_DIR) {
  $NapCatDir = $env:NAPCAT_DIR
} else {
  $localNapCat = Join-Path $PSScriptRoot 'napcat\NapCat.Shell'
  if (Test-Path $localNapCat) {
    $NapCatDir = $localNapCat
  } else {
    $NapCatDir = Join-Path (Split-Path -Parent $PSScriptRoot) 'napcat\NapCat.Shell'
  }
}
...
if (Test-Port 3001) {
  ...
  if ($launchArgs.Count -gt 0) {
    $argStr = $launchArgs -join ' '
    Start-Process -FilePath $launcher -ArgumentList $argStr -WorkingDirectory $NapCatDir -WindowStyle Minimized
  } else {
