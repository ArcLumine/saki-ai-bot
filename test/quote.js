...
console.log('\n【8】★★ 阈值可关（`chat.quoteAfterGap: 0`）+ 调用方要求仍然优先');
{
  ...
  const priv = { message_type: 'private', user_id: OWNER, self_id: BOT, message: [textSeg('在吗')], sender: { nickname: '<主人>' } };
...
console.log('\n【12】★★ 正文里**点名叫她** → 也算召唤（<主人>：「明确提到祥子的没有回复」）');
{
  ...
  // ⑦ 私聊不受影响（私聊本来就回）
  const priv = { message_type: 'private', user_id: OWNER, self_id: BOT, message: [textSeg('祥子')], sender: { nickname: '<主人>' } };
