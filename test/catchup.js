...
{
  const b = makeBot([
    { message_id: 1, time: sec(2 * 60 * 1000), user_id: OWNER, sender: { user_id: OWNER }, message: at('我可以蹭一下你吗') },
    { message_id: 2, time: sec(5 * 60 * 1000), user_id: OWNER, sender: { user_id: OWNER }, message: at('求互相包容') },
  ]);
...
console.log('\n【2】★★ 不该补的四种（这是防重复/防刷屏的关键）');
{
  const b = makeBot([
    ...
    { message_id: 11, time: sec(-5000), user_id: OWNER, sender: {}, message: at('上线之后的') },
    // ② 超过 10 分钟的（用户指定）
    { message_id: 12, time: sec(15 * 60 * 1000), user_id: OWNER, sender: {}, message: at('十五分钟前的') },
    // ③ 纯闲聊（没 @、没关键词、不是服务器问题）
    { message_id: 13, time: sec(3 * 60 * 1000), user_id: OWNER, sender: {}, message: txt('今天天气不错') },
    // ④ 她自己发的
...
{
  const b = makeBot([
    { message_id: 21, time: sec(60 * 1000), user_id: OWNER, sender: {}, message: txt('那个整合包在哪下') },
    { message_id: 22, time: sec(90 * 1000), user_id: OWNER, sender: {}, message: txt('服务器现在在线人数多少') },
  ]);
  const n = await b.catchUpMissed();
...
{
  const b = makeBot([
    { message_id: 31, time: sec(60 * 1000), user_id: OWNER, sender: {}, message: at('同一条') },
  ]);
  const recent = await import('../src/recent.js');
  recent.clear(G1);
  recent.remember({ message_type: 'group', group_id: G1, user_id: OWNER, message_id: 31, message: at('同一条') }, { text: '同一条' });
  const n = await b.catchUpMissed();
...
{
  const b = makeBot([
    { message_id: 41, time: sec(60 * 1000), user_id: OWNER, sender: {}, group_id: G1, message: at('群里问的') },
    { message_id: 42, time: sec(70 * 1000), user_id: OWNER, sender: {}, group_id: G2, message: at('另一个群问的') },
  ]);
  const n = await b.catchUpMissed();
