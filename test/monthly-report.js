...
// ⚠️⚠️ 2026-09-23 加：给**隔离账本声明一个起始月**。
//     闸门（`epochDay`）原来只按"文件生日"推起点 —— 在一个**新 clone 出来的仓库**里，
//     src/ 和 state/ 的生日就是今天，于是本套件造的那笔 **2026-09-10** 会被当成
//     "比机器人诞生还早的脏数据"丢掉 ⇒ 【7】【8】的"相比上月"必红
//     （症状：`delta=null｜上月（2026-09）账本上没有记录`；实测 41/59 那次就是它）。
//     账本顶层的 `epoch` 就是为这种情况准备的（spend.js 的 `applyLedgerEpoch()`）——
//     让套件自己声明，就不会再依赖"跑测试的机器上文件有多旧"。
writeFileSync(join(ROOT, T.spend), JSON.stringify({ epoch: '2026-01', days: {} }, null, 2), 'utf8');
spend.reload();
