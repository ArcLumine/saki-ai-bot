...
{
  // ── 上限与续补概率 ────────────────────────────────────
  // ⚠️ 2026-09-19 用户反馈"追补太频繁"，上限已从 5 降到 2（see src/follow-up.js）。
  check(fu.MAX_PER_REPLY === 2, '默认上限 = 2（用户反馈"太频繁"后收紧）');
...

{
  ...
  check((await fu.askFollowUp({ mainReply: '' })) === '', '正文为空 → 返回空（不发请求）');
  check(
    (await fu.askFollowUp({ mainReply: '这样就行吗？' })) === '',
...

{
  // ── status() 给管理界面用 ─────────────────────────────
  const s = fu.status();
  check(s.enable === true && s.maxPerReply === 2, `status() 反映配置：${JSON.stringify(s)}`);
}
