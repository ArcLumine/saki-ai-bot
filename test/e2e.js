...
async function main() {
  ...
  console.log('[5] 验证私聊白名单（无需 @）');
  const privateRepliesBefore = received.filter((m) => m.action === 'send_private_msg').length;
  const modelCallsBeforePrivate = llmCalls.length;
  botSocket.send(
    JSON.stringify({
      post_type: 'message',
      ...
      user_id: '40004',
      self_id: SELF_ID,
      message: [{ type: 'text', data: { text: 'this private sender is not allowed' } }],
      sender: { user_id: '40004', nickname: '陌生人' },
    }),
  );
  await sleep(1200);
  check(
    received.filter((m) => m.action === 'send_private_msg').length === privateRepliesBefore,
    '不在私聊白名单的用户不会收到回复',
  );
  check(llmCalls.length === modelCallsBeforePrivate, '不在私聊白名单的用户不会触发模型调用');
  ...
  check(gotPrivate, '白名单用户的私聊消息得到回复');
