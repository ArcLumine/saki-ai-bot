...
console.log('\n【1】状态机：这一段对话的计数与分段');
{
  let conv = d.userTurn(null, { uid: OWNER, name: '<主人>', text: '在吗', now: T0 });
  ...
  conv = d.botTurn(conv, { uid: OWNER, name: '<主人>', text: '在。', now: T0 + 5000 });
  check(conv.herTurns === 1 && conv.theirTurns === 1, '她说话 → 她 1 句');
  check(conv.lastBotText === '在。' && conv.lastBotReplyTo === OWNER, '记下她上一句 + 她在回谁');
  check(conv.lastBotReplyAt === T0 + 5000, '兼容字段 `lastBotReplyAt` 还在（别的地方在读）');
  check(conv.followUpChain === 1, '兼容字段 `followUpChain` 还在（防刷屏计数）');

  conv = d.userTurn(conv, { uid: OWNER, text: '籽岷最近有什么视频', now: T0 + 20000 });
  check(conv.theirTurns === 2 && conv.herTurns === 1, '同一段里继续累加');

  // ⚠️ 隔太久（> 10 分钟）→ 算**新的一段**，计数从头来，别把半小时前的对话算进来
  const far = d.userTurn(conv, { uid: OWNER, text: '还在吗', now: T0 + 40 * 60 * 1000 });
  check(far.theirTurns === 1 && far.herTurns === 0, '隔了 40 分钟 → **新的一段**（计数归零）');
}

console.log('\n【2】状态机：三种"谁在说话"（him / other / alone）');
{
  const conv = d.botTurn(d.userTurn(null, { uid: OWNER, text: '在吗', now: T0 }), {
    uid: OWNER,
    text: '在。',
    now: T0 + 5000,
  });

  const him = d.snapshot(conv, { now: T0 + 25000, uid: OWNER, idleMs: 30000, sameUserMs: 180000 });
  ...
  const late = d.snapshot(conv, { now: T0 + 90000, uid: OWNER, idleMs: 30000, sameUserMs: 180000 });
  check(late.who === 'him' && late.inWindow === false, '★ 隔了 85 秒（超 idleMs）**还是 him** —— 这次不许再漏接');

  const over = d.snapshot(conv, { now: T0 + 190000, uid: OWNER, idleMs: 30000, sameUserMs: 180000 });
  check(over.phase === 'idle' && over.who === 'alone', '超过 sameUserMs → 这段散了（idle）');

  const none = d.snapshot(null, { now: T0, uid: OWNER });
  check(none.phase === 'idle' && none.who === 'alone', '没有对话 → idle / alone（不炸）');
}

console.log('\n【3】喂给说话判断的那段状态（#4）');
{
  const conv = d.botTurn(d.userTurn(null, { uid: OWNER, name: '<主人>', text: '在吗', now: T0 }), {
    uid: OWNER,
    name: '<主人>',
    text: '在。',
    now: T0 + 5000,
  });
  const conv2 = d.userTurn(conv, { uid: OWNER, text: '籽岷最近有什么视频', now: T0 + 60000 });
  const txt = d.describe(conv2, { now: T0 + 65000, uid: OWNER, idleMs: 30000, sameUserMs: 180000 });
